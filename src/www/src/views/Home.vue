<template>
  <!-- eslint-disable -->
  <div class="home">
    <header>
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 class="text-2xl font-bold leading-tight text-gray-900">
            Site Details
          </h2>
        </div>
      </header>
      <main class="mt-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 lg:flex">
          <div class="w-full lg:w-60 mr-8 mb-6">
            <nav>
              <a href="#" class="group flex items-center px-3 py-2 text-sm leading-5 font-medium text-gray-900 rounded-md bg-gray-300 hover:text-gray-900 focus:outline-none focus:bg-gray-300 transition ease-in-out duration-150">
                <svg class="flex-shrink-0 -ml-1 mr-3 h-6 w-6 text-gray-600 group-hover:text-gray-600 group-focus:text-gray-700 transition ease-in-out duration-150" stroke="currentColor" fill="none" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l9-9 9 9M5 10v10a1 1 0 001 1h3a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1h3a1 1 0 001-1V10M9 21h6"/>
                </svg>
                <span class="truncate">
                  App
                </span>
              </a>
              <a href="#" class="mt-1 group flex items-center px-3 py-2 text-sm leading-5 font-medium text-gray-600 rounded-md hover:text-gray-900 hover:bg-gray-300 focus:outline-none focus:text-gray-900 focus:bg-gray-200 transition ease-in-out duration-150">
                <svg class="flex-shrink-0 -ml-1 mr-3 h-6 w-6 text-gray-400 group-hover:text-gray-500 group-focus:text-gray-600 transition ease-in-out duration-150" stroke="currentColor" fill="none" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path>
                </svg>
                <span class="truncate">
                  Notifications
                </span>
              </a>
              <a href="#" class="mt-1 group flex items-center px-3 py-2 text-sm leading-5 font-medium text-gray-600 rounded-md hover:text-gray-900 hover:bg-gray-300 focus:outline-none focus:text-gray-900 focus:bg-gray-200 transition ease-in-out duration-150">
                <svg class="flex-shrink-0 -ml-1 mr-3 h-6 w-6 text-gray-400 group-hover:text-gray-500 group-focus:text-gray-600 transition ease-in-out duration-150" stroke="currentColor" fill="none" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                </svg>
                <span class="truncate">
                  Environment
                </span>
              </a>
              <a href="#" class="mt-1 group flex items-center px-3 py-2 text-sm leading-5 font-medium text-gray-600 rounded-md hover:text-gray-900 hover:bg-gray-300 focus:outline-none focus:text-gray-900 focus:bg-gray-200 transition ease-in-out duration-150">
                <svg class="flex-shrink-0 -ml-1 mr-3 h-6 w-6 text-gray-400 group-hover:text-gray-500 group-focus:text-gray-600 transition ease-in-out duration-150" stroke="currentColor" fill="none" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 8v8m-4-5v5m-4-2v2m-2 4h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                </svg>
                <span class="truncate">
                  Logs
                </span>
              </a>
              <a href="#" class="mt-1 group flex items-center px-3 py-2 text-sm leading-5 font-medium text-gray-600 rounded-md hover:text-gray-900 hover:bg-gray-300 focus:outline-none focus:text-gray-900 focus:bg-gray-200 transition ease-in-out duration-150">
                <svg class="flex-shrink-0 -ml-1 mr-3 h-6 w-6 text-gray-400 group-hover:text-gray-500 group-focus:text-gray-600 transition ease-in-out duration-150" stroke="currentColor" fill="none" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"></path>
                </svg>
                <span class="truncate">
                  Meta
                </span>
              </a>
            </nav>
          </div>
          <div class="flex-1">
            <div v-if="status.is_deployed">
              <deployment class="mb-8"></deployment>
              <deploy-script class="mb-8"></deploy-script>
              <deploy-trigger class="mb-8"></deploy-trigger>
              <deploy-webhooks class="mb-8"></deploy-webhooks>
              <maintenance-mode class="mb-8"></maintenance-mode>
              <deploy-branch class="mb-8"></deploy-branch>
              <git-remote class="mb-8"></git-remote>
              <uninstall class="mb-8"></uninstall>
            </div>

             <div v-else>
              <install-repository class="mb-8"></install-repository>
            </div>
          </div>
        </div>
      </main>
  </div>
</template>

<script>
// @ is an alias to /src
import axios from 'axios';

import InstallRepository from '@/components/InstallRepository.vue';
import Deployment from '@/components/Deployment.vue';
import DeployScript from '@/components/DeployScript.vue';
import DeployTrigger from '@/components/DeployTrigger.vue';
import DeployWebhooks from '@/components/DeployWebhooks.vue';
import MaintenanceMode from '@/components/MaintenanceMode.vue';
import DeployBranch from '@/components/DeployBranch.vue';
import GitRemote from '@/components/GitRemote.vue';
import Uninstall from '@/components/Uninstall.vue';

export default {
  name: 'Home',
  components: {
    InstallRepository,
    Deployment,
    DeployScript,
    DeployTrigger,
    DeployWebhooks,
    MaintenanceMode,
    DeployBranch,
    GitRemote,
    Uninstall,
  },
  data() {
    return {
      status: {
        is_deployed: null,
      },
    };
  },
  methods: {
    getStatus() {
      const path = '/api';
      axios.get(path)
        .then((res) => {
          this.status = res.data;
        })
        .catch((error) => {
          // eslint-disable-next-line
          console.error(error);
        });
    },
  },
  created() {
    this.getStatus();
  },
};
</script>
